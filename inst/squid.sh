...
if cat /etc/os-release | grep PRETTY_NAME | grep "Ubuntu 19.04"; then
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid/squid.conf https://www.dropbox.com/s/pp1j4juui7uegvm/squid.conf
...
/sbin/iptables -I INPUT -p tcp --dport 3128 -j ACCEPT
    ...
    /usr/bin/wget --no-check-certificate -O /etc/squid3/squid.conf https://www.dropbox.com/s/pp1j4juui7uegvm/squid.conf
    /sbin/iptables -I INPUT -p tcp --dport 8080 -j ACCEPT
    /sbin/iptables-save
    service squid3 restart
    ...
    echo "SISTEMA OPERATIVO NO SOPORTADO POR FAVOR PONGASE EN CONTACTO CON @ALEX_DROID9_0_MX"
    exit 1;
fi
